...
INDEX_MEMORY_SCRIPT = Path(
    os.getenv("INDEX_MEMORY_SCRIPT", "/app/memory-proxy/index_memory.py")
)
...
def main():
    ...
    print(f"Index script: {INDEX_MEMORY_SCRIPT}", flush=True)
    ...
    if not INDEX_MEMORY_SCRIPT.exists():
        print(f"❌ index_memory.py not found: {INDEX_MEMORY_SCRIPT}", flush=True)
        sys.exit(1)
